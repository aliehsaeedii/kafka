...
class DualColumnFamilyAccessor implements RocksDBStore.ColumnFamilyAccessor {
    ...
    private byte[] get(final RocksDBStore.DBAccessor accessor, final byte[] key, final Optional<ReadOptions> readOptions) throws RocksDBException {
        final byte[] newValue = readOptions.isPresent() ? accessor.get(newColumnFamily, readOptions.get(), key) : accessor.get(newColumnFamily, key);
        if (newValue != null) {
            return newValue;
        }

        final byte[] oldValue = readOptions.isPresent() ? accessor.get(oldColumnFamily, readOptions.get(), key) : accessor.get(oldColumnFamily, key);
        if (oldValue != null) {
    ...
    /**
     * An iterator that merges two RocksDB column families, converting values from the old format
     * to the new format on-the-fly using the provided conversion function.
     */
    private static class RocksDBDualCFIterator extends org.apache.kafka.common.utils.AbstractIterator<KeyValue<Bytes, byte[]>>
        ...
        public synchronized boolean hasNext() {
            if (!open) {
                throw new org.apache.kafka.streams.errors.InvalidStateStoreException(String.format("RocksDB iterator for store %s has closed", storeName));
            }
    ...
    /**
     * A range iterator that merges two RocksDB column families within a specified range,
     * converting values from the old format to the new format on-the-fly.
     */
