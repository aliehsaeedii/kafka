  ...
  private class DualColumnFamilyAccessor implements ColumnFamilyAccessor {
    ...
    private byte[] get(final DBAccessor accessor,
      ...
      if (legacyValue != null) {
        ...
        final byte[] converted = convertToHeaderFormat(legacyValue);
    ...
    @Override
    public void commit(DBAccessor accessor, Map<TopicPartition, Long> changelogOffsets) throws RocksDBException {
      ...
      if (nextLegacy == null && !iterLegacy.isValid()) {
        ...
        if (nextWithHeaders == null) {
          next = KeyValue.pair(
              ...
              convertToHeaderFormat(iterLegacy.value())
          );
          nextLegacy = null;
          if (forward) {
            iterLegacy.next();
